...
To resolve this problem, a `while loop`[^1] has been added. If the inputted amount is negative, the function prints an error message and skips to the next iteration of the while loop using the `continue` statement. Otherwise, it `breaks out`[^2] of the loop and prompts the user to enter the second amount in a similar way.
...
Furthermore, a `Try` and `Excpet` block[^3] has been added to handle cases where the user inputs a non-integer value, such as a floating number, and raise a `ValueError` exception with a custom error message.
<details>
...
One of the challenges of this program was finding a way to output the *€* *sign*. After a research on the web I found helpful information on **Stack** **Overflow**[^4] website whch offered various opton to solve the problem. I choose to incorporate the € Unicode glyph number into my code.
<details>
...
This program is designed to show the last `4 digits` of a 10 digit bank account number, replacing the first 6 digits with an `'X'`[^5]. However, the program's limitation was that it could not display the account numbers of any length, as it always showed only the last 4 digits. To overcome this limitation and fulfill the extra requirement, a different approach was adopted taking the percentages as reference.
...
The `"if"` statement uses the modulus operation to determine if the number is *even*; if the remainder is zero, the program divides the number by two and prints it. Otherwise, the `"else"` statement multiplies the number by three and adds one, and prints the result. To make the output neater, the `end= " "`parameter[^6] is used to write the output in a single line.

### *Weekday* *Checker*

> Write a program, called **weekday.py**, that outputs whether or not today is a weekday.

In order to complete the task, it was necessary to import the `datetime` module[^7], so we could manipulate date and time.
Thanks to the documentation of **geeksforgeeks**, it was able to find the missing piece to crack the code.
There it was discovered that by using `date.isoweekday()`[^8], where *Monday* is *1* and **Sunday** is **7**, it is simple to determine whether today is a weekday or a weekend with the aid of an if expression.
...
As an extra, using the conditional statement `if`, I included a check to ensure that the user's input is a positive floating-point number. If the user input is negative, the number is converted to a positive value using the `abs()` function[^9].
<details>
<summary>Input</summary>
...
To round the float number to 3 decimal points, the `round ()`[^10] has been used.
...
The code uses two differents libraries, `numpy`[^11] & `matplotlib.pyplot`[^12] to create a plot.
...
The Normal distrbution can be found using the *random.normal()* functon[^13], while range for the *h(x)=x3* function the*arange()* function from numpy library can be used.
...
[^2]: Python Break and Python Continue – How to Skip to the Next Function - [Freecodecamp](https://www.freecodecamp.org/news/python-break-and-python-continue-how-to-skip-to-the-next-function/)
[^3]: Python Try Except - [w3schools](https://www.w3schools.com/python/python_try_except.asp)
[^4]: Use Unicode glyph number: [StackOverflow](https://stackoverflow.com/questions/39935857/how-can-i-print-a-euro-%E2%82%AC-symbol-in-python)
[^5]: is there a better way to mask a credit card number in python? [StackOverflow](https://stackoverflow.com/questions/9730653/is-there-a-better-way-to-mask-a-credit-card-number-in-python) was used to understand better how to mask the first 6 digits of a Bank Account number.
[^6]: Python end parameter in print - [Geeksforgeeks](https://www.geeksforgeeks.org/gfact-50-python-end-parameter-in-print/)
[^7]: Python datetime module - [Geeksforgeeks](https://www.geeksforgeeks.org/python-datetime-module/)
[^8]: Isoweekday Function In Python - [pythontic](https://pythontic.com/datetime/date/isoweekday)
[^9]: abs in Python - [Geeksforgeeks](https://www.geeksforgeeks.org/abs-in-python/)
[^10]: Limit floats to two decimal point - [Pythonhow](https://pythonhow.com/how/limit-floats-to-two-decimal-points/#:~:text=To%20limit%20a%20float%20to,resulting%20in%20the%20value%203.14.)
[^11]: Numpy - [Numpy](https://numpy.org/)
[^12]: Matplotlib pyplot [W3schools](https://www.w3schools.com/python/matplotlib_pyplot.asp)
[^13]: How to lot normal distribution over histogram in python [geeksforgeeks](https://www.geeksforgeeks.org/how-to-plot-normal-distribution-over-histogram-in-python/)
